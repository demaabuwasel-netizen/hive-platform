      ...
      <div className="min-h-screen bg-[#FFF7E6]">
        ...
        <div className="max-w-2xl mx-auto px-6 py-10">
          <EmptyState
            ...
            description="This match doesn't exist or may have been removed. Head back to see all available matches."
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        >
          <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 flex-1">
              <AvatarDisplay src={student.avatar} name={student.name} size="lg" />
              <div>
                <p className="font-bold text-[#0D183D]">{student.name}</p>
                <p className="text-sm text-[#4B6382]">{student.field}</p>
                <p className="text-xs text-navy-400">{student.university}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row-reverse items-center gap-3 flex-1">
              <AvatarDisplay src={ngo.imageUrl || ngo.avatar} name={ngo.name} size="lg" />
              <div className="sm:text-right">
                <p className="font-bold text-[#0D183D]">{ngo.name}</p>
                <p className="text-sm text-[#4B6382]">{ngo.location}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-5 pt-5 border-t border-[rgba(13,24,61,0.08)]">
            ...
            <p className="text-navy-600 text-sm leading-relaxed">{match.headline}</p>
          </div>
        </motion.div>
        ...
        >
          ...
          <p className="text-xs text-navy-400 mb-5">
            Each factor below reflects how the AI interpreted both profiles — not just surface-level keywords.
          </p>
        ...
        >
          ...
          <p className="text-xs text-navy-400 mb-5">
            These questions are tailored to this specific pairing to help the NGO verify the match.
          </p>

          <div className="flex flex-col gap-3">
            {match.suggestedQuestions.map((q, i) => (
              <motion.div
                key={i}
        ...
        >
          {interested ? (
            ...
            >
              ...
              <p className="font-semibold text-emerald-700 mb-1">Interest sent to {ngo.name}</p>
              <p className="text-emerald-600 text-sm">They'll be in touch if there's a mutual fit.</p>
            </motion.div>
          ) : (
            <button
              onClick={() => setInterested(true)}
              className="btn-honey text-base px-8 py-3.5 w-full sm:w-auto"
            >
        ...
        <div className="grid sm:grid-cols-2 gap-6 mt-6">
          ...
          >
            ...
            <p className="text-navy-600 text-sm leading-relaxed">{student.summary}</p>
            <div className="flex flex-wrap gap-1.5 mt-3">
              {student.skills.slice(0, 4).map(s => (
                <span key={s} className="bg-navy-50 text-navy-700 text-xs px-2.5 py-1 rounded-full">{s}</span>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="card"
          >
            <p className="text-xs font-semibold text-navy-400 uppercase tracking-wider mb-3">NGO Summary</p>
            <p className="text-navy-600 text-sm leading-relaxed">{ngo.summary}</p>
            <div className="flex flex-wrap gap-1.5 mt-3">
              {ngo.tags.map(t => (
                <span key={t} className="bg-[#FFF7E6] text-navy-600 text-xs px-2.5 py-1 rounded-full border border-cream-300">{t}</span>
              ))}
            </div>
          </motion.div>
        </div>
