...
export default function RolesList({ roles, selectedRoleId, onSelectRole, loading }) {
  ...
  function RoleCard({ role, index, expandedCard = false }) {
    ...
    return (
      <motion.button
        ...
        className={`group relative h-[92px] overflow-hidden rounded-[24px] border px-4 py-4 text-left transition-all hover:-translate-y-0.5 after:pointer-events-none after:absolute after:inset-x-3 after:top-0 after:h-14 after:rounded-t-[inherit] after:bg-[radial-gradient(circle_at_18%_0%,rgba(255,255,255,0.32),transparent_48%)] ring-1 ring-white/55 backdrop-blur-2xl ${
          expandedCard ? 'w-full' : 'w-[268px] shrink-0'
        } ${
          isActive
            ? 'border-transparent bg-[linear-gradient(135deg,rgba(232,240,254,0.88),rgba(220,234,255,0.62))] shadow-[0_10px_24px_rgba(26,115,232,0.09),0_1px_0_rgba(255,255,255,0.9)_inset]'
            : 'border-[#DADCE0]/70 bg-[linear-gradient(135deg,rgba(255,255,255,0.9),rgba(255,255,255,0.66))] shadow-[0_10px_24px_rgba(32,33,36,0.045),0_1px_0_rgba(255,255,255,0.94)_inset] hover:border-[#C9CED6]/80 hover:bg-[linear-gradient(135deg,rgba(255,255,255,0.97),rgba(255,255,255,0.78))] hover:shadow-[0_13px_30px_rgba(32,33,36,0.055),0_1px_0_rgba(255,255,255,0.97)_inset]'
        }`}
      >
  ...
  return (
    <section
      className={`mx-auto mb-6 w-full max-w-[1360px] min-w-0 rounded-[30px] border border-white/90 bg-[linear-gradient(135deg,rgba(255,255,255,0.68),rgba(248,251,255,0.34))] px-4 pb-3 pt-4 shadow-[0_24px_64px_rgba(26,115,232,0.085),0_1px_0_rgba(255,255,255,0.96)_inset,0_-1px_0_rgba(26,115,232,0.025)_inset] backdrop-blur-2xl transition-all duration-300 ${
              ...
              <AnimatePresence mode="wait" initial={false}>
                {expanded ? (
                  ...
                  <motion.div
                    ...
                    <div ref={scrollerRef} className="flex max-w-full snap-x snap-mandatory gap-3 overflow-x-auto pb-3 pl-16 pr-12 pt-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                      {roles.map((role, i) => (
