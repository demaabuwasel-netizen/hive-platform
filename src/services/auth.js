...
export async function getUserRow(userId, { signal } = {}) {
  ...
  try {
    ...
    const { data, error } = await withQueryTimeout(q, 10000, 'getUserRow SELECT')
    if (error) {
...
export async function ensureUserRow(authUser, { signal } = {}) {
  ...
  try {
    ...
    if (signal) q = q.abortSignal(signal)
    const { data, error } = await withQueryTimeout(q, 10000, 'ensureUserRow SELECT')
  ...
  try {
    ...
    if (signal) q = q.abortSignal(signal)
    const { data, error } = await withQueryTimeout(q, 10000, 'ensureUserRow INSERT')
